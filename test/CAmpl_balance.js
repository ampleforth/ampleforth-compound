...
const MINT_cAMPL_SUPPLY = AMPLS_LOCKED_UP.div(INITIAL_EXCHANGE_RATE);
...
contract('CAmpl', function (accounts) {
  beforeEach('setup CAmpl contract', async function () {
    ...
    await cAmpl.mint(AMPLS_LOCKED_UP, {from:owner});
  });
  ...
  describe('when rebase does not change AMPL supply', function () {
    ...
    it('should update the underlying ampl balance', async function () {
      expect(await cAmpl.balanceOfUnderlying.call(owner)).to.eq.BN(AMPLS_LOCKED_UP);
      expect(await ampl.balanceOf.call(cAmpl.address)).to.eq.BN(AMPLS_LOCKED_UP);
    });
  });
});
